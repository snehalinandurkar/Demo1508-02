/*
 * CMSSvcPasswordChangeAction.java
 * 
 *
 * Created on December 7, 2011, by Jordan Fernandez
 * Part of CorDaptix Web Self Service (10321)
 * Cloned PasswordChangeAction.java
...
public class CMSSvcPasswordChangeAction
    ...
    private void showEntryForm(HttpServlet servlet, HttpServletRequest request, HttpServletResponse response)
        ...
        if (reminder.retrieveData() && !reminder.getPasswordQuestion().equals("null")) {
            ...
            if(isNull(session.getAttribute("superUser"))){
            	...
            	jspName = Util.getCMFile("CMSSvcPasswordChangeView.jsp", context);
            }
    ...
    private void processRequest(HttpServlet servlet, HttpServletRequest request, HttpServletResponse response)
        ...
        if (reminder.getPasswordPlain().equals(request.getParameter("priorPassword"))) {
            ...
            if (!newPassword.equals(request.getParameter("newPasswordConfirm"))) {
                ...
                try {
                    if(WebStringUtilities.checkXssInjection(newPassword)) {
                        String error = "Invalid character found on New Password.";
                        String jspName = Util.getCMFile("SSvcPasswordChange.jsp", context);
                        forward(jspName + "?errorMsg=" + error, request, response);
                    }
                }
                ...
                // Prior Password Validation
                try {
                    if(WebStringUtilities.checkXssInjection(request.getParameter("priorPassword"))) {
                        String error = "Invalid character found on Prior Password.";
                        String jspName = Util.getCMFile("SSvcPasswordChange.jsp", context);
                        forward(jspName + "?errorMsg=" + error, request, response);
                    }
                }
                ...
                try {
                    if(WebStringUtilities.checkXssInjection(request.getParameter("newPasswordConfirm"))) {
                        String error = "Invalid character found on New Password Confirm.";
                        String jspName = Util.getCMFile("SSvcPasswordChange.jsp", context);
                        forward(jspName + "?errorMsg=" + error, request, response);
                    }
                }
                ...
                try {
                    if(WebStringUtilities.checkXssInjection(request.getParameter("passwordAnswer"))) {
                        String error = "Invalid character found on Password Answer.";
                        String jspName = Util.getCMFile("SSvcPasswordChange.jsp", context);
                        forward(jspName + "?errorMsg=" + error, request, response);
                    }
                }
    ...
    private void forward(String url, HttpServletRequest request, HttpServletResponse response) throws IOException,
            ServletException {
        System.out.println("CMSSvcPasswordChangeAction [" + url + "]");
